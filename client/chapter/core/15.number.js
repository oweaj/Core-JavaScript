...
// 여러 수 중, 최댓값
let max = Math.max(10, 50, 34);
console.log(max);

// 여러 수 중, 최솟값
let min = Math.min(10, 50, 34);
console.log(min);

let arr = [35, 656, 23, 6, 32];
...
function getRandomMinMax(minNum, maxNum) {
  return Math.round(Math.random(minNum) * maxNum);
}
console.log(getRandomMinMax(1, 10));

let getRandomMinMax1 = (min, max) => {
  if (min > max) throw new Error("최소값이 최대값보다 큽니다 다시한번 확인해주세요!!");
  return Math.round(Math.random() * (max - min) + min);
};

console.log(getRandomMinMax1(2, 10));
